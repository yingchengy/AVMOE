...
from nets.net_trans_v3 import MMIL_Net
...
def train(args, model, train_loader, optimizer, criterion, criterion_event, epoch):
	...
	for batch_idx, sample in enumerate(train_loader):
		...
		# optimizer.zero_grad()
		if args.is_cmbs:
			is_event_scores, event_scores, av_score, adapter_index_dict = model([wave], image, rand_train_idx=rand_train_idx, stage='train')
			...
			loss = loss_is_event + loss_event_class + loss_cas
		else:
			output = model([wave], image, rand_train_idx=rand_train_idx, stage='train')
		...
		if ((batch_idx + 1) % args.accum_itr == 0) or (batch_idx + 1 == len(train_loader)):
			optimizer.step()
			optimizer.zero_grad() # 修复 2024.5.12
...
def eval(model, val_loader, args):
	...
	for batch_idx, sample in enumerate(val_loader):
		...
		with torch.no_grad():
			...
			if args.is_cmbs:
				is_event_scores, event_scores, av_score, adapter_index_dict = model([wave], image)
				is_event_scores = is_event_scores.transpose(1, 0).squeeze(-1).contiguous()
				acc = compute_accuracy_supervised(is_event_scores, event_scores, labels)
				mean_acc = (mean_acc * batch_idx + acc) / (batch_idx + 1)
	...
	if args.is_cmbs:
		...
		print("saving expert_activation_counts")
		# 保存 expert_activation_counts
...
def main():
	...
	if args.mode == 'train':
		...
		if args.is_init_best_checkpoint==1:
			print('loading pre-training!!!!!!!!!!!!!!!!!')
			model.load_state_dict(torch.load("models/" + "best_82.18" + ".pt"), strict=False)
	########## note for fast training #########
		...
		print("load : ", args.model_save_dir)
		test_dataset = AVE_dataset(opt=args, mode='test')
		test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=1, pin_memory=True)
		model.load_state_dict(torch.load(args.model_save_dir), strict=False)
